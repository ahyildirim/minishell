...
static int complete_arg(t_lexlist *lex_table)
{
	char	*str;

	str = get_arg(); //pipe'dan sonraki argümanı al.
	if(!str)
		return (1);
	create_lexlist(str, &lex_table); //yeni alınan argüman için lexer'ı çalıştır.
	set_lex_types(lex_table); //türünü belirle.
	str_append_char(&g_data.input, ' '); //input'a buradaki str'ı strjoin ile ekleyeceğin için sonuna bir boşluk ekle
	ft_strjoin(&g_data.input, str); //input ile str'ı birleştir.
	free(str);
...
static int	check_pipe_error(t_lexlist *lex_table)
{
...
static int	check_errors(t_lexlist *lex_table)
{
	int	pipe_error;

	if(lex_table == g_data.lex_table && lex_table->type == SIGN_PIPE) //Bu fonksiyona gelen node ilk node'mu kontrolü yap ve yazılan ilk eleman (ilk node) type'ı eğer PIPE(|) ise error ver (| cat).
	{
	...
	{
		pipe_error = check_pipe_error(lex_table); //pipe için error kontorlü yap.
		if (pipe_error < 0)
			return (0);
...
void	check_syntax(void)
{
	...
	while(lex_table)
	{
		error = check_errors(lex_table); //Error kontrolüne ilk başladığımız fonksiyon.
		if (error == -1) //Hata ile karşılaşıldıysa döngüden çık.
			break;
		else if(error == 0) //Hata ile karşılaşılmadıysa bir sonraki iterasyona geç.
			continue;
		if(g_data.lex_table) //Node varsa bir sonraki node'a geç, yoksa NULL ata.
